...
# will try to increase the font size here  .. below uses getFontMeterics
#    didn't work ........... gave up
#        current_font = self.text_pane.font
#        current_size = current_font.size
#        current_name = current_font.name
#        current_style = current_font.style
#        if self.DEBUG == True: sys.stderr.write("current font.name:"  + str(current_name) )
#        if self.DEBUG == True: sys.stderr.write("current font.style:"  + str(current_style) )
#        if self.DEBUG == True: sys.stderr.write("current font.size:"  + str(current_size) )
#        new_size = current_size + 4
#       You can use Font.PLAIN, Font.BOLD, Font.ITALIC for style
#        new_font = Font(current_font.name, Font.PLAIN, new_size)
#        new_font = Font(current_font.name, current_style, new_size)
#        # Set the new font in the text pane using Jython property access
#        self.text_pane.font = new_font # didn't work
#        self.text_pane.updateUI() # didn't work
...
# but it doesn't work. through testing I proved that runsource does no buffering the ... can be treated as an error. or just ignored
...
#        source = "\tprint('x')\nelse:\n\tprint('y')\n" # this should complete the command >>> more should be true
        ...
        if not self.more:
              if self.DEBUG == True: sys.stderr.write("console.py running runsource \n")
...
####        """ Prints the prompt """
    ...
    def __propertiesChanged(self):
        ...
        size = 18 #jEdit.getIntegerProperty("jython.fontsize", 14)
