...
# need this -> sys.path.append(r'/home/wayne/source/jythonconsole-0.0.7x/') for console.py etc. imports to work includeing the '/' on the end
...
__author__ = "Don Coleman <dcoleman@chariotsolutions.com>" # modified by waynelloydsmith@gmail.com
...
#    PROMPT = '>>>'
#    PROCESS = '...'
    BANNER = ["Jython Completion Shell", InteractiveConsole.getDefaultBanner()] # was already using InteractiveConsole ????
    DEBUG = True
...
#        if self.DEBUG == True : sys.stderr.write("insertText\n")
...
#        sys.stderr.write("getText\n")
#        sys.stderr.write(source3)        
        return source3
...
# I think all the buffer stuff below was an attempt to put the >>> commands and the ... commands together
# but it doesn't work.
...
#        source = "+".join(self.buffer)
        source = "\n".join(self.buffer) # replace what we just got with whats in the buffer all the items in the list ????
                                        # above converts the list to a string with \n as a separator
        source = source + "\n"          # there was a \n on both ends of it ???
#       source = self.buffer
        self.last_key_pressed_was_enter = True
#  some testing
#        source = "if True:\n    print('x')\nelse:\n    print('y')\n"  # push() was happy with this and printed x
#        source = "if False:\n    print('x')\nelse:\n    print('y')\n"  # runsource was happy with this and printed y
#        source = "if (1 == 1 ):\n\tprint('x')\nelse:\n\tprint('y')\n"  # try tabs runsource was happy with this and printed x
        source = "if (1 == 1 ):"                                        # try removing all the \n from the input  ... this worked \n is a bad thing..
        more = self.interp.push(source) # execute the command with push
        if self.DEBUG == True: sys.stderr.write("more " + str(more) + "\n")
        source = "\tprint('x')"
        more = self.interp.push(source) # execute the command with push
        if self.DEBUG == True: sys.stderr.write("more " + str(more) + "\n")
        source = "else:"
        ...
        if self.DEBUG == True: sys.stderr.write("more " + str(more) + "\n")
        source = "\tprint('y')\n"                                           # put a \n on the last line
        more = self.interp.push(source) # was runsource now push
        if self.DEBUG == True: sys.stderr.write("more " + str(more) + "\n")

        if self.DEBUG == True: sys.stderr.write('source \n'+source) # source already has a \n on the other end of it
        if self.DEBUG == True: sys.stderr.write('buffer '+str(self.buffer)+'\n')

        if more:
            self.printOnProcess() # ... and we save it in  the buffer for next the time enter is pressed
        else:
            self.resetbuffer()
            self.printPrompt()  # >>>
...
#        if self.DEBUG == True: sys.stderr.write("replaceRow\n")
    ...
    def backSpaceListener(self, event=None):
        ...
        onFirstPosition = self.text_pane.getCaretPosition() <= ( self.__lastLine()[0] + len(Console.PROMPT) )
#        if onFirstPosition:
...
class Interpreter(InteractiveConsole): # was InteractiveInterpreter .. changed to InteractiveConsole dec 29 2025
    def __init__(self, console, locals):
        InteractiveConsole.__init__(self, locals) # changed to InteractiveConsole dec 29 2025
